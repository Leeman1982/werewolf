...
static const char* const arpPatternNames[NUM_ARP_PATTERNS] = {
...
public:
  ...
  // WerewolfAmyS3: tick-driven stepping — the master clock calls this once
  // per arp step; returns the MIDI note to play now, or -1 for a rest.
  int stepNote() {
    if (!isPlaying || noteCount == 0) return -1;
    // Rest patterns
    if (pattern == ARP_GATES && (currentStep % 4) == 3) { currentStep++; return -1; }
    if (pattern == ARP_EUCLIDEAN_5_8 || pattern == ARP_EUCLIDEAN_7_12) {
      int len = (pattern == ARP_EUCLIDEAN_5_8) ? 8 : 12;
      bool hit = euclideanRhythm[currentStep % len];
      if (!hit) { currentStep++; return -1; }
    }
    ...
    if (idx >= 0 && idx < noteCount) {
      ...
      else if (maxOctaves > 1) note += ((currentStep / noteCount) % maxOctaves) * 12;
      if (note > 127) note -= 12 * ((note - 116) / 12);
    }
    currentStep++;
    return note;
  }
